...
namespace RestlessDb.Managers
{
    public class GenericQueryManager
    {
        ...
        public GenericQueryManager(ILogger<GenericQueryManager> logger, QueryRepository queryRepository, QueryItemsRepository queryItemsRepository,
            ...
            Dictionary<string, object> queryParameters)
        {
            ...
            var queryItemExt = queryItemsRepository.LoadQueryItemExt(queryName);

            // check the query parameters with the QueryParamsProvider
            var checkedParmeters = queryParamsProvider.CollectParamsForQuery(queryItemExt.Sql, queryParameters, new List<Dictionary<string, object>>());
            ...
            ret.MetaData = queryItemExt.AsQueryMetaData();
        ...
        {
            logger.LogInformation("GetQueryResults-Child SQL: {0}", queryItemExt.Sql);

            List<Dictionary<string, object>> childRows;
            bool hasMoreRows;

            var effectiveQueryParams = queryParamsProvider.CollectParamsForQuery(queryItemExt.Sql, queryParameters, parentRowStack);
            ...
            parentRowStack.Last().Add(queryItemExt.Name, childRows);
